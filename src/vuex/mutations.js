...
// 设置电数据
export const SET_Elect_INFO = (state, seckills) => {
    state.page.totalpage = seckills.PageCount;
    console.log(state.page.totalpage)
    state.DeviveItem = seckills.Data;
    var list = [];
...
// 设置返回的状态值
export const SET_Elect_UPDATE = (state, seckills) => {
    state.BackState = seckills;
};

//获取第一个下拉框的数据
export const SET_Elect_ITEM = (state, seckills) => {
    state.FirstSelectItem = seckills;
};

//获取第二个下拉框的数据
export const SET_ECOND_ITEM = (state, seckills) => {
    state.SecondSelectItem = seckills;
};